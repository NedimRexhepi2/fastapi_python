...
async def update_user(user_id: int,user:UserModelEdit, db: Annotated[Session, Depends(get_session)]):
...
async def create_user_db(user: UserModelCreate, db: Annotated[Session, Depends(get_session)]):
    result=db.execute(
